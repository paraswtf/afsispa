...
// 	FanoutTargets   = 5            // limit number of target peaks per anchor (speeds indexing)
...
// 	win := hann(FFTSize)
// 	// frames
// 	if len(samples) < FFTSize { return nil, nil }
...
// 		for k := minBin; k <= maxBin; k++ {
// 			mag := S[t][k]
// 			if mag < -PeakAbsThresh { // below threshold relative to max
// 				continue
// 			}
...
// 					if S[tt][kk] > mag { isMax = false; break }
// 				}
// 			}
// 			if isMax {
// 				peaks = append(peaks, Peak{T: t, F: k, A: mag})
// 			}
// 		}
// 	}
// 	return S, peaks
...
// 	for _, p := range peaks { byTime[p.T] = append(byTime[p.T], p) }
// 	// precompute a sparse list of times having peaks
// 	ts := make([]int, 0, len(byTime))
// 	for t := range byTime { ts = append(ts, t) }
// 	sort.Ints(ts)
...
// 			limit := FanoutTargets
// 			if limit > len(cands) { limit = len(cands) }
// 			for i := 0; i < limit; i++ {
// 				b := cands[i]
// 				dt := b.T - a.T
// 				if dt <= 0 { continue }
